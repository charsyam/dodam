...
#[tokio::test]
async fn executes_comma_join_with_not_in_subquery_sql() {
    let tempdir = tempfile::tempdir().expect("tempdir");
    let orders_path = tempdir.path().join("orders.parquet");
    let customers_path = tempdir.path().join("customers.parquet");
    write_orders_parquet(&orders_path);
    ...
    let sql = format!(
        "SELECT o.id FROM '{}' AS o, '{}' AS c WHERE o.customer_id = c.id AND c.id NOT IN (SELECT id FROM '{}' WHERE name = 'bob') ORDER BY o.id",
        orders_path.display(),
        customers_path.display(),
        customers_path.display()
    );
    let output = execute_sql(&DodamEngine::default(), &sql, 2)
        .await
        .expect("execute comma join with NOT IN subquery sql");
