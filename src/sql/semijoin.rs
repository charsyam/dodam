...
pub(super) fn semijoin_exists_key_pair(
    conjuncts: &[SqlExpr],
    inner_alias: &str,
    outer_alias: &str,
...
pub(super) async fn collect_semijoin_key_set(
    engine: &DodamEngine,
    path: PathBuf,
    key_column: &str,
...
pub(super) fn semijoin_membership_mask(
    batch: &RecordBatch,
    key_column: &str,
    keys: &SemijoinKeySet,
) -> Result<BooleanArray> {
...
pub(super) fn invert_boolean_array(array: &BooleanArray) -> BooleanArray {
    BooleanArray::from(
        array
            .iter()
...
pub(super) fn semijoin_outer_projection(
...
pub(super) fn semijoin_i64_key_at(column: &ArrayRef, row: usize) -> Result<Option<i64>> {
    if column.is_null(row) {
        return Ok(None);
    }
